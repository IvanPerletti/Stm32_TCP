...
#include "string.h"
#endif
#define Q_SIZE 16		//queue max size: when reached no more elements are enqued
#define Q_LIMIT (Q_SIZE + 1)

template <typename Q_Type, int maxSize = Q_SIZE> class IQ_Generic {
...
public:
	...
	void push(Q_Type* msg)
	{
		...
		if ( u8Index != u8TopIdx)
		{
			memcpy(&queue[u8BottomIdx % maxSize], msg, sizeof(Q_Type));
			u8BottomIdx = u8Index;
		}
	}
    //-----------------------------------------------------------------------
	...
	long pop(Q_Type* msg = (Q_Type*)(0))
	{
		...
		{
			if (msg != 0) {
				memcpy(msg, &queue[u8TopIdx % maxSize], sizeof(Q_Type));
				result = 1;
			}
			u8TopIdx = u8Index;
		}
	...
	/**
	 * @brief extract block of elements (from te top)
	 * @param msg	destination buffer
	 * @param num	amount of elements to extract
	 * @return number of extracte elements
	 */
	long pop(Q_Type* msg, int num)
	{
		...
		const uint8_t u8Index =  (u8TopIdx + num)%maxSize;
		if ( !isEmpty() )
		{
			if (msg != 0) {
				memcpy(msg, &queue[u8TopIdx % maxSize], num * sizeof(Q_Type));
				result = num;
			}
			u8TopIdx = u8Index;
		}
	...
	long flush(Q_Type* dst, int maxNum = -1)
	{
		...
		memcpy(dst, queue, maxNum * sizeof(Q_Type));
		this->reject(maxNum);
